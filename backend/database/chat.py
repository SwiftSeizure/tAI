...
import os
from backend.models import ChatResponse,ClientErrorResponse
...
def queryBot(studentID :int, path: str, prompt: str, session:Session) -> ChatResponse:
    ...
    try:
        ...
        return ChatResponse(conversationID = conversation.id,studentID=studentID,messages=conversation.messages,responses=conversation.responses) 
