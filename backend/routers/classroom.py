...
from backend.models import ClassroomResponse
...
def get_teacher_home(accountID: int, session: DBSession) -> ClassroomResponse:
    db_units = classroom_db.get_class_units(accountID, session) 
    units = [HomeClass(id=c.id, name=c.name) for c in db_units] # type: ignore
